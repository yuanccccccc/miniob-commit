...
RC PredicatePushdownRewriter::rewrite(std::unique_ptr<LogicalOperator> &oper, bool &change_made)
{
  ...
  change_made = false;
  // 开始pushdown
  if (child_oper->type() == LogicalOperatorType::TABLE_GET) {  // predicate的子算子是table get
  ...
  if ((predicate_expr == nullptr)) {
    // 所有的表达式都下推到了下层算子,这个predicate operator其实就可以不要了。但是这里没办法删除，弄一个空的表达式吧
